...
describe('build report', () => {
    ...
    test.each(formats.filter(({ format }) => format !== 'json'))
    ('should throw error when build report if node status is unknown', ({ format }) => {
        const obj = [ makeNode('key', 'random_1234', 'value') ];
        expect(() => buildReport(obj, format)).toThrow();
    });
});